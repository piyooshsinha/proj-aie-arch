...
.venv/bin/python -m pytest -q          # 33 tests, no network needed
...
export `ANTHROPIC_API_KEY`, and run `aie.api.app:app` instead of the demo.
...
| Observability      | `aie/observe/trace.py`        | Built. Span tree, cost, latency, counters                 |
...
Streaming, an eval harness and online scoring, real vector search, an agent loop
over the action registry, authentication, and durable stores. The interfaces for
each are in place; see the table above.
